...
| C3-R12 | A single happy-path integration smoke test exercises classify + extract end-to-end against the live API for one sample document. Comprehensive HTTP-seam coverage out of scope. | Tagged JUnit test, gated on `ANTHROPIC_API_KEY`; comprehensive HTTP-seam coverage with stubbed LLM seams is delegated to the scenario suite (C3-R17). |
...
| C3-R17 | Scenario tests boot the real Spring stack with stubbed `LlmClassifier` / `LlmExtractor`; run inside `make test`; no API key required. | Scenario test class executes inside `./gradlew check`; passes without `ANTHROPIC_API_KEY` set. |
| C3-R18 | Fixture YAML schema defined; loader validates and fails fast on malformed fixtures (unknown properties, missing required keys, conflicting input shapes). | Loader unit test asserts rejection of unknown properties, missing required keys, and conflicting input shapes. |
