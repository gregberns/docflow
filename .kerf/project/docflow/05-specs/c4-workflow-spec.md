...
| C4-R6 | `Resolve` from `Review` (with `workflowOriginStage` set): no type change → `clearFlag` and return to `workflowOriginStage`; with type change → set `reextractionStatus = IN_PROGRESS`, emit `DocumentStateChanged`, invoke `C3.LlmExtractor.extract(documentId, newDocTypeId)` (asynchronous). On `ExtractionCompleted` → UPDATE `detectedDocumentType` + `extractedFields`, set `reextractionStatus = NONE`, `clearFlag` (currentStage stays `Review`), emit `DocumentStateChanged`. On `ExtractionFailed` → set `reextractionStatus = FAILED`, emit `DocumentStateChanged`. | Integration test driving both paths |
